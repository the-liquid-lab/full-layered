...
# define sigma_kappa(eta, i)					\
  (sigma_n[i]*(eta[i+1] + eta[i-1] - 2.*eta[i])/sq(Delta))/rhom
#else // dimension == 2
# define sigma_kappa(eta, i)						\
  ((sigma_d.x[i]*(0.2*(eta[i+1,1] + eta[i-1,1] - 2.*eta[i,1] +		\
    ...
    sigma_n[i]*(eta[i+1,1] + eta[i-1,-1] -				\
		eta[i+1,-1] - eta[i-1,1]))/sq(Delta))/rhom
#endif // dimension == 2


#define p_cap(eta,i) (sigma_kappa(eta, i))
#define a_cap(eta, i)						\
  (gmetric(i)*(p_cap (eta, i) - p_cap (eta, i - 1))/Delta)
/** 
not a great way of doing this as it doesn't work if you have other accelerations
*/

//#define a_tot(eta,i) (a_baro(eta,i) + a_cap(eta,i))
...
event face_fields (i++)
{
  ...
  foreach_face (reduction (min:dtmax)) {
    ...
    if (H > dry) {
      double c = um/CFL + sqrt((G + max(sigma[], sigma[-1])/rhom*sq(pi/Delta))*
